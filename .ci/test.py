...
def run_one(p: Plugin) -> bool:
    ...
    # bit).
    subprocess.check_call([pip_path, 'install', *pip_opts, *global_dependencies])

    # Now install all the requirements
    print("Installing requirements from {p.requirements}".format(p=p))
    subprocess.check_call([pip_path, 'install', '-U', *pip_opts, '-r', p.requirements])

    if p.devrequirements.exists():
        print("Installing requirements from {p.devrequirements}".format(p=p))
        subprocess.check_call([pip_path, 'install', '-U', *pip_opts, '-r', p.devrequirements])

    assert pytest_path.exists()

    print("Running tests")
    try:
        subprocess.check_call([pytest_path, p.path, '-vvv', '-n 5', '--timeout=90'])
        return True
    except:
        return False
    finally:
